...
const numOfQuestions = 10; // Możesz dostosować liczbę pytań w quizie

const fallbackQuestions: Question[] = [
  // Dodany awaryjny zestaw dla najmłodszych
...
export default function Quiz({ goBack, onSaveScore }: { goBack: () => void, onSaveScore: (g: string, t: string, n: number, tm: number | null) => void }) {
  ...
  // NOWE STANY: Do obsługi neonowego cooldownu
  ...
  useEffect(() => {
    ...
    fetch('/quiz.txt?t=' + Date.now())
      .then(res => {
        if (!res.ok) throw new Error(`Błąd serwera: ${res.status}. Pliku nie ma w folderze public!`);
        return res.text();
      })
      .then(text => {
  ...
  // ZAKTUALIZOWANA FUNKCJA ODPOWIEDZI
  const handleAnswer = (index: number) => {
    ...
    // Ustawiamy timer na 4 sekundy, zanim przejdziemy dalej
    setTimeout(() => {
